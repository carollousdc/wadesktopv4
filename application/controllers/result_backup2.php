    ...
    public function check_result() {
                ...
                foreach ($seasons as $key => $value) {
                    if (is_numeric($value)) {
                        ...
                        foreach ($matches_format as $value_format) {
                            if ($value_format[0] == 'CB' && strpos($angka_result, $matches_value_format[0][0])) {
                                $user_win[$value_master->id] = 1;
        ...
        if (!empty($user_win)) {
            foreach ($user_win as $key_result => $val_result) {
                ...
                if (!empty($result_angka_format[$key_result])) {
                    foreach ($result_angka_format[$key_result] as $value) {
                        $hasil = $value;
                        // $totalhd4a += $result_hasil_format[$key_result];
                        $hadiah = $hasil * $this->master->price_pool($angka_result, $value, $get_kontak[$key_result]);
